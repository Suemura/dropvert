...
- **アニメーション WebP → `SKIP` され、フレーム数が変わらないこと**
...
- **アニメーション WebP を `sips` に通すと 1 フレームに潰れます**。同じ形式へ書き戻す経路（`sips` で PNG 中間 → `cwebp`）でこれをやると、アニメーションが失われた出力で元ファイルがゴミ箱へ行きます。`convert.sh` は同形式パスに限り `ANIM` チャンクを見て `SKIP` します
