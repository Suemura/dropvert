...
- `VERSION` — バージョン番号（SemVer）を 1 行だけ書いたファイル。**バージョンの唯一の出どころ**で、読んで書き込むのは `build.sh` だけ（`Info.plist` の `CFBundleShortVersionString` と `CFBundleVersion` に同じ値を入れる）。`git describe` を使わないのは、cask がソースの tarball を展開してビルドする経路に `.git` が無いため。読み込みと検証は既存の `.app` を消す**前**に置く
