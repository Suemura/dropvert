...
- 1 件だけのドロップでも破綻しないこと
- 200 件を超えるドロップ → 誤って「ゴミ箱への移動に失敗」が出ないこと（`trash.js` は成功時も空行を 1 行返すため、`xargs` の分割数だけ空行が積み上がる。空行は落としてから判定している）
- 進捗ウィンドウの「停止」を押す → 変換済みの分だけ元ファイルがゴミ箱へ移動し、通知が「中止 —」で始まること。未着手のファイルは変換もされず削除もされないこと
