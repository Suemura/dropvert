...
CI（`.github/workflows/ci.yml`、`macos-latest`）が push / PR ごとに自動で回すのは **`./lint.sh`・`./tests/run.sh`・ビルド + 署名検証**です。`convert.sh` や `run.sh` を変更したら、手で試す前に `./tests/run.sh` を回してください（34 ケース、10 秒ほど）。
