...
ファイルをアイコンにドラッグ&ドロップすると、**元と同じフォルダ**に変換後の画像を作成し、生成を確認できたものだけ元ファイルをゴミ箱へ移動します。設定画面はありません。ドロップするだけです。
...
3. 元と同じフォルダに `.webp` が作られる
4. 生成を確認できたファイルがゴミ箱へ移動する
5. 結果が通知される（失敗があればダイアログでファイル名と理由を表示）

進捗表示の「停止」を押すと、そこまでに変換できた分だけを残して中止します。未着手のファイルは変換もされず、ゴミ箱へも移動しません。通知は「中止 — 4 件変換 / 26 件未処理」のように表示されます（`.webp` を投げたときの「スキップ」とは区別されます）。
...
`Dropvert.applescript` の先頭にある設定を編集し、`./build.sh` を再実行してください。
...
複数ファイルをまとめて変換する場合は `run.sh` を使います。第 1 引数は 1 行 1 パスのファイルリスト、第 3 引数は並列度です。
