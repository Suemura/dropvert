...
- **元の場所に出力** — 保存先を選ぶ操作がない
- **安全な後始末** — 変換に成功したファイルだけをゴミ箱へ移動（Finder 経由なので「元に戻す」が使える）。失敗したファイルは削除しない
...
> **初回のみ**: ゴミ箱への移動に Finder の操作権限が必要なため、macOS が確認ダイアログを表示します。許可してください。
...
```
dropvert/
├── Dropvert.applescript  # droplet 本体。ドロップ受付、ゴミ箱への移動、結果通知
├── convert.sh            # 1 ファイルを変換するロジック
├── build.sh              # osacompile でアプリをビルド
├── README.md
├── CLAUDE.md
└── LICENSE
```
