...
#
# ただし入力リストが見つからないときだけは、TMP と FAIL の 2 行で終わる
# (CONVERTED / SKIPPED / UNPROCESSED は出ない)。呼び出し側はパースの前に
# 3 つのカウンタを 0 で初期化すること。
#
...
if [ ! -f "$listfile" ]; then
	# ここだけは 3 つのカウンタを出さずに終わる (冒頭の契約を参照)。
	# 呼び出し側 (AppleScript) はパースの前に 0 で初期化するので実害は無い。
